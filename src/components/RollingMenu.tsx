...
export default function RollingMenu() {
  ...
  return (
    <div className="flex items-center gap-2" ref={ref as any}>
      <motion.button
        animate={{ rotate: expanded ? 45 : 0 }}
        aria-label="Navigation Menu"
        className="h-10 w-10 flex items-center justify-center select-none rounded-full bg-black p-3 dark:bg-white"
        onClick={handleClick}
        whileTap={{ scale: 1.1 }}
      >
            ...
            {memoizedActions.map((action, i) => (
              ...
              >
                <Comp
                  ...
                  href={action.section === 'Socials' ? action.href ?? undefined : undefined}
                  to={action.section === 'Navigation' ? action.href ?? undefined : undefined}
                  key={i}
                  onClick={
