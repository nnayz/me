...
export default function PostDisplayer({
  ...
  const getCarouselInfo = () => {
    if (post.media_type === 'CAROUSEL_ALBUM' && post.children?.data) {
      ...
      const videoChildren = post.children.data.filter(
  ...
  return (
    ...
    >
      <div className="overflow-hidden rounded-sm border border-black/10 bg-white dark:border-white/10 dark:bg-neutral-900">
        {/* Image Section */}
        <div className="relative aspect-square bg-black/5 dark:bg-white/5">
          ...
          {carouselInfo && carouselInfo.hasMultiple && (
            <div className="absolute right-2 top-2 flex items-center gap-1 rounded bg-black/70 px-2 py-1 text-xs text-white">
              ...
              <span>
                {carouselInfo.currentIndex + 1} / {carouselInfo.totalItems}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4 p-4 sm:p-6">
          ...
          {post.caption && (
            <div className="space-y-1">
              <h3 className="text-tertiary text-xs font-semibold uppercase tracking-wide">
                Caption
              </h3>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
                    <div>
                      ...
                      <span className="ml-1 font-medium text-primary">
                        {carouselInfo.totalItems}
                      </span>
                    </div>
                    <div>
                      <span className="text-tertiary">Images:</span>
                      <span className="ml-1 font-medium text-primary">
                        {carouselInfo.totalImages}
                      </span>
                    </div>
                      ...
                      >
                        ...
                        {index === carouselInfo.currentIndex && (
                          <div className="absolute inset-0 flex items-center justify-center bg-primary/10">
                            <div className="rounded bg-primary px-1.5 py-0.5 text-[10px] text-white">
                              Current
                            </div>
                          </div>
                        )}
